...
for path in glob.glob(path, recursive=True):
    ...
    for line in lines:
        ...
        if "#pragma HLS INTERFACE" in line:
            ...
            groups = re.findall(r'(\w+\s*)=(\s*\w+)', line)
            for group in groups:
                key, val = group
                params[key.strip()] = str(val).strip()
    ...
    output = template.render(class_name=function_name.capitalize(), axilites=axilite_list, aximasters=aximaster_list, axistreams=axistream_list)

    #auto-ident shitty code generated by jinja
    output, _ = FormatCode(output)
